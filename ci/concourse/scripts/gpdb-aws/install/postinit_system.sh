...
echo "export MASTER_DATA_DIRECTORY=${MASTER_DIRECTORY}/gpseg-1" >> ~gpadmin/.bashrc
echo "export MASTER_DATA_DIRECTORY=${MASTER_DIRECTORY}/gpseg-1" >> ~root/.bashrc

echo "Adding ETL hosts to pg_hba.conf"
for IP in $INTERNAL_ETL_IPS; do
  echo -e "host\tall\tgpadmin\t${IP}/32\ttrust" >> "${MASTER_DIRECTORY}/gpseg-1/pg_hba.conf"
done
