...
generateBuildInfo() {
    ...
    echo "Copy build.out to build-prod..."
...
getBuildId() {
    ...
    #Get the buildID
    ./buildDB.py -p 5432 -h build-prod -d cdbunit -u postgres addBuild buildIdInfo.txt > ${BUILD_ID} 2>&1
    if [ $? -ne 0 ] ; then
...
buildTestUpdate() {
    ...
    BUILD_DB_OPTS="-p 5432 -h build-prod -d cdbunit -u postgres"
