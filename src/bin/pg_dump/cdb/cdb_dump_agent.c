		...
		if (g_fout->remoteVersion >= 80205)
		{
			...
			if (rejlim && strlen(rejlim) > 0)
			{
				...
				if (errtblname && strlen(errtblname) > 0)
				{
					...
					char *tablename = fmtId(tbinfo->dobj.name);

					/* Check errortable was not generated by LOG ERRORS statement.
					 * To do: depracate the use of LOG ERRORS INTO
					 */
					char tmp[500];
					snprintf(tmp, sizeof(tmp), "%s%s", errtablename, EXT_PARTITION_NAME_POSTFIX);
					char *tmpStr= Safe_strdup(tmp);
	...
	else
	{
		...
		if (g_gp_supportsPartitioning)
		{
			...
			appendPQExpBuffer(query, "SELECT "
						...
						"WHERE partitionschemaname != schemaname AND tablename = E%s;",
						fmtId_with_single_quote(tbinfo->dobj.name));
...
addDistributedBy(PQExpBuffer q, TableInfo *tbinfo, int actual_atts)
{
	...
	/* Always single quote strings in the where clause with fmtId_with_single_quote(), escape special characters */
	appendPQExpBuffer(query,
				"SELECT attrnums from pg_namespace as n, pg_class as c, gp_distribution_policy as p "
				"WHERE c.relname = E%s ", fmtId_with_single_quote(tbinfo->dobj.name));

	appendPQExpBuffer(query,
				"AND n.nspname=E%s "
				"AND c.relnamespace=n.oid "
				"AND c.oid = p.localoid",
				tbinfo->dobj.namespace->dobj.name != NULL ? fmtId_with_single_quote(tbinfo->dobj.namespace->dobj.name) : c_PUBLIC);
