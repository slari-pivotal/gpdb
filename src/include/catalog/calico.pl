...
=head1 VERSION
 ...
 This document describes version 71 of calico.pl, released
 Mon Feb 25 14:53:57 2013.
...
Options:
    ...
    -metadata       json document (catdump) describing the catalog
...
=item B<-metadata> <filename> (Required)
    ...
    the metadata file is a json document (catdump) describing the catalog generated by tidycat
...
=item B<-logquery>
    ...
    Turn on logging (elog) for all basic functions.  Use caqltrack.sql or
the caqltrack test suite to process these statements from the logs to
...
Based on the work of Dr. Richard Astley:
...
The CaQL parsing and code generation is driven by the tidycat
catdump.json metadata file, which describes the catalog tables, their
...
=head2 "Physical" Catalog

The "physical" catalog tables, i.e. the "persistent" tables and other
storage-related tables, are not covered by CaQL.
...
pg_shdescription
	...
	$glob_glob->{stripout} = '/* **stripout** */';
	$glob_glob->{striprex} = quotemeta($glob_glob->{stripout});
...
BEGIN {
	    ...
	    my $s_metadata;                  # json document (catdump) describing the catalog
    ...
    GetOptions(
		...
		'metadata|json|catdump=s'    =>     \$s_metadata,
	...
	$glob_glob->{_sleazy_properties}->{version} = '71';
	...
	$glob_glob->{_sleazy_properties}->{slzy_date} = '1361832837';
    ...
    my $bigstr = <<'EOF_bigstr';
{
   "args" : [
      ...
      {
         "alias" : "json|catdump",
         "long" : "the metadata file is a json document (catdump) describing the catalog generated by tidycat",
         "name" : "metadata",
         "required" : "1",
         "short" : "json document (catdump) describing the catalog",
         "type" : "file"
      },
      {
         "alias" : "perf",
...
if ((!pCtx->cq_setlockmode) &&
	...
	/* MPP-18955: only check on master or if in writer gang */
...
#			SNAPSHOT	 => "SnapshotNow", 
	...
	for my $ii (0..(scalar(@wpred2)-1))
	{
		...
		# save the bind variable types for later analysis
		push @{$bqh->{bindvar_types}}, $pred2->{wktyp};
...
# for a cql query, make sure has >= 5 bind variables, and that
# arguments match
sub check_bind_variables
{
	...
	if (!scalar(@args))
	{
		if (scalar(@bindlst))
		{
			...
			warn("found $bindnum bind variables but no args");
			return 0;
		}
	}
	else
	{
		# if arg is just a quoted string, maybe it is continuation of
		# cql query?
		if ($args[0] =~ m/^\s*\".*\"/)
		{
			my $a2 = $args[0];
...
# used by check_datum_type() to match the bind variable column type to
# a "GetDatum()" wrapper function.
sub ctype_to_getdatum
{
	my $ctype = shift;
	...
	my %c2getdatum	   =  
		(
		 bool		   => "BoolGetDatum",
		 CString	   => "CStringGetDatum", # ???
		 char		   => "CharGetDatum",
		 Float4		   => "Float4GetDatum",
		 Float8		   => "Float8GetDatum",
		 int2		   => "Int16GetDatum",
		 int4		   => "Int32GetDatum",
		 int8		   => "Int64GetDatum",
		 int1		   => "Int8GetDatum",
		 NameData	   => "NameGetDatum",
		 Oid		   => "ObjectIdGetDatum",
		 oid		   => "ObjectIdGetDatum", # XXX XXX ??
		 oidvector	   => "PointerGetDatum",
		 uint2		   => "UInt16GetDatum",
		 uint4		   => "UInt32GetDatum",
		 uint8		   => "UInt64GetDatum",
		 uint1		   => "UInt8GetDatum"
		);

	unless (exists($c2getdatum{$ctype}))
	{
		warn "no conversion for type: $ctype";
		return undef;
	}

	return $c2getdatum{$ctype};
} # end ctype_to_getdatum

# MPP-18975: 
sub check_datum_type
{
	my ($datumlst, $bvtyplst, $qerr) = @_;

	my $maxi = scalar(@{$datumlst});

	if ($maxi != scalar(@{$bvtyplst}))
	{
		print Data::Dumper->Dump($datumlst);
		print Data::Dumper->Dump($bvtyplst);

		warn "bind / arg mismatch";
		return 0;
	}
	...
	for my $ii (0..($maxi-1))
	{
		...
		# NOTE: When the bind variable *doesn't* have a GetDatum
		# wrapper, it must have a placeholder "cqlIsDatumFor<type>()"
		# wrapper macro (which does nothing but assuage the type
		# checker).  Currently this case is *exceptionally* rare, ie 1
		# case out of 990, which uses cqlIsDatumForCstring.  If
		# necessary, define the wrapper macros for other types (see
		# ctype_to_getdatum()) in catquery.h

		if ($dtm =~ m/cqlIsDatumFor/)
		{
			# convert to <type>GetDatum
			$dtm =~ s/cqlIsDatumFor(\w*)/$1GetDatum/;
		}

		unless ($dtm =~ m/GetDatum/)
		{
			die "$qerr: cannot check type for $dtm";
		}

		my $dtm2 = ctype_to_getdatum($bvt);
		...
		while (1)
		{
			...
			if ($dtm2 eq "NameGetDatum")
			{
				# try again with cstring
				$dtm2 = "CStringGetDatum";
				next;
			}

			warn "expected $dtm2 for $bvt, got $dtm";
			return 0;
		}
...
sub get_caql
{
	...
	# sigh.  I thought I could use Regexp::Common or Text::Balanced
	# but I am too lame.  So use brute force -- split the whole file
	# and count parens!
	my @bigsplit = split(/((?:\s)*cql(?:0)?)|(\))|(\()/, $whole_file);

#	print Data::Dumper->Dump(\@bigsplit);
	...
	# Note: need to skip over undefs/empty strings in this mess...
	while ($ii < scalar(@bigsplit))
	{
		my $bb = $bigsplit[$ii];
		...
		else # no match yet
		{
			...
			if (($bb) =~ m/(\s)*cql(0)?/)
			{
				...
				# need to skip undefs, blanks
				while ($jj < scalar(@bigsplit))
				{
					if (!defined($bigsplit[$jj]))
					{
						$jj++;
						next;
					}
					if (0 == length($bigsplit[$jj]))
					{
						$jj++;
						next;
					}
					last # leave unless matches "("
						if ($bigsplit[$jj] !~ /\(/);
					...
					# found start of cql and first paren
					$cqlstr = "$bb(";
					$parenC = 1;
		...
		$ii++;
	} # end while ii < scalar bigsplit
		
	die "filename $filnam: failed to close expression $cqlstr" 
		if (defined($cqlstr));
...
#		$lin =~ s/\"\s*$^\s*"//gm;
		...
		$lin =~ s/\)\s*$//; # trim the trailing paren
...
#	print Data::Dumper->Dump(\@baz);
	...
	for my $lin (@baz)
	{
		...
		@zzz = split(/\"/, $lin, 3);
		# zzz[0] is 'cql("'
		my $qry	   = $zzz[1]; # no quotes
		my $arglst = $zzz[2]; # which may start with a comma...

		$caqlh->{queries}->{$qry} = { 
			files => {}, cql => "cql0", arglists => [] }
				unless (exists($caqlh->{queries}->{$qry}));

		# detect case of pure cql0 queries
		if ($caqlh->{queries}->{$qry}->{cql} eq "cql0")
		{
			# "cql" takes precedence (if was already a "cql0")
			$caqlh->{queries}->{$qry}->{cql} = "cql"
				if ($lin =~ m/cql\(/);
		}

		# check arg list for cql queries
		if ($lin =~ m/cql\(/)
		{
			die "bad caql in $filnam: $lin"
				unless (check_bind_variables($qry, $arglst));

			# save the arglist so can check against bindvar types later
			push @{$caqlh->{queries}->{$qry}->{arglists}}, 
				   {file=>$filnam, arglist=>$arglst};
		}
...
if (!(pCtx)->cq_externrel) { \
{CAQL_HEAPCLOSE}(pCtx)->cq_heap_rel = InvalidRelation; } else
...
#
# includes the basic functions, the caql_switch, 
# and, optionally: iud_switch/locking, dawdle
# 
sub do_gperf
{
	my ($caqlh, $gpf, $bigh) = @_;
...
#							  $caqlh->{basic}->{$bq}->{iud_func_name},
	...
	# check the types 
	# NOTE: this only works if the input arguments 
	# have XXXGetDatum(...) wrappers!!
	if (1)
	{
		use Text::ParseWords;

		for my $qryk (sort(keys(%{$caqlh->{queries}})))
		{
			my $qry = $caqlh->{queries}->{$qryk};
			my $bq  = $caqlh->{basic}->{$qry->{basic}};

			# array of bind variable types from basic query
			my $bvt = $bq->{bindvar_types};
			
			next
				unless (defined($bvt) &&
						scalar(@{$bvt}));

			# list of arglists in query
			for my $arglh (@{$qry->{arglists}})
			{
				my $argf = $arglh->{file};
				my $argl = $arglh->{arglist};
				
				# split by comma, but use Text::ParseWords::parse_line to
				# preserve quoted descriptions
				my @args = parse_line(",", 1, $argl);
				shift @args; # skip first arg (due to extra lead comma)
				
				# XXX XXX: pg_aoseg - skip segrelid/segidxid
				# XXX XXX: pg_aoblkdir - skip blkdirrelid/blkdiridxid
				# NOTE: kind of gross
				if ($bq->{tablename} =~ m/^(gp\_aoseg|gp\_aocsseg|gp\_aoblkdir)$/)
				{
					while (scalar(@args))
					{
						last 
							if ($args[0] !~ m/(segrelid|segidxid|blkdirrelid|blkdiridxid)/);
				...
				my $cdqerr = "file $argf, query $qryk";

				die "bad datum type in file $argf, query $qryk"
					unless check_datum_type(\@args, $bvt, $cdqerr);
			}
		}
	}
	...
	if (exists($glob_glob->{gperf}) && $glob_glob->{gperf} )
	{
		...
		## remove preprocessor line number directives for Hitoshi
		# eg: #line 10 "gperf.init"
		$gperfout =~ 
			s/^\#line\s+(\d)*\s+\"gperf\.init\"//gm;
