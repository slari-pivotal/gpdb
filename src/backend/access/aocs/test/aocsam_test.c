...
test__aocs_begin_headerscan(void **state)
{
	...
	/*
	 * AppendOnlyStorageRead_Init assigns
	 * storageRead->storageAttributes.  will_assign_*() functions
	 * mandate a paramter as an argument.  Here we want to set
	 * selective members of a parameter.  I don't know how this can be
	 * achieved using cmockery.  This test will be meaningful only
	 * when we are able to set storageAttributes member of
	 * desc.ao_read.
	 */
