...
explain update purchase_par set region = 'new_region' where id in (select m.b from m, r where m.a = r.b) and day in (select a from r);
...
explain update purchase_par set month = month+1 from r,m where purchase_par.id = m.b and purchase_par.month = r.b;
...
-- heap table: delete --
explain delete from r where b in (select month-1 from purchase_par,m where purchase_par.id = m.b);
...
explain delete from m using r,purchase_par where m.a = r.b and m.b = purchase_par.id;
...
explain update m set b = m.b + 1 from r,purchase_par where m.a = r.a and m.b = purchase_par.id;
