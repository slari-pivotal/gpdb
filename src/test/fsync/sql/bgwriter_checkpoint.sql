...
-- Set bgwriter_lru_percent to max value so that bgwriter does not
-- skip the pages we are about to make dirty.
\!gpconfig -c bgwriter_all_percent -v 100 --skipvalidation | grep 'completed successfully' | sed -e 's/^.*INFO..//'
\!gpstop -au | grep -i 'error\|fail'
...
-- seconds.
select wait_for_bgwriter('fsync_test1'::regclass, 10) as passed;
